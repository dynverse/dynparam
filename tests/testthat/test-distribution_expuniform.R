...
test_that("expuniform_distribution errors when expected", {
  expect_error(expuniform_distribution(lower = -Inf, upper = 0), "lower is not a single numeric value in \\]-Inf,Inf\\[")
  expect_error(expuniform_distribution(lower = NA, upper = NA), "is not a single numeric value in \\]-Inf,Inf\\[")
  expect_error(expuniform_distribution(lower = ~help_us, upper = "we need more wood"), "is not a single numeric value in \\]-Inf,Inf\\[")
  expect_error(expuniform_distribution(lower = 0, upper = Inf), "upper is not a single numeric value in \\]-Inf,Inf\\[")
